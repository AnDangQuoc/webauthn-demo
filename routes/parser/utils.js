...
module.exports = {
  ...
  objectSlice: function (object, keys) {
    const result = {}
  ...
  findKeyWithPath: function (object, key) {
    if (key in object) {
      return {
        path: key,
        value: object[key],
  ...
  findAllKeys: function (object, key) {
    const result = []
  ...
  prettyStringify: function (object) {
    return JSON.stringify(object, null, 2)
  },

  x5cArrayToCertInfo: function (array) {
    const info = []
    for (const x5c of array) {
      ...
      const slice = {
        ...
        subject: cert.subject.typesAndValues.map(
          (v) => v.value.valueBlock.value
        ),
